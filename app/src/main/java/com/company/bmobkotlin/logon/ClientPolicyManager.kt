...
class ClientPolicyManager(private var sapWizardApplication: SAPWizardApplication) {
    ...
    fun setClientPolicy(jsonObject: JSONObject?, secureStoreManager: SecureStoreManager) {
        val passcodePolicyJson: JSONObject? = jsonObject?.optJSONObject(SETTINGS_PASSCODE)
        ...
        val logSettingsJson = jsonObject?.optJSONObject(SETTINGS_LOG)
        ...
        secureStoreManager.doWithPasscodePolicyStore { passcodePolicyStore ->
                                passcodePolicyStore.put(KEY_CLIENT_POLICY, policyFromServer)
                                passcodePolicyStore.put(KEY_RETRY_COUNT, 0)
                        }
    ...
    private inner class PolicyCallbackListener(private val downloadLatch: CountDownLatch) : Settings.CallbackListener {
        ...
        override fun onSuccess(result: JSONObject) {
            setClientPolicy(result, secureStoreManager)
            ...
            if (isLogPolicyEnabled) {
                AppLifecycleCallbackHandler.getInstance().activity?.runOnUiThread {
                    ...
                    Toast.makeText(activity,
                        String.format(activity!!.getString(R.string.log_level_changed), sapWizardApplication.logUtil.getLevelString(finalLogLevel)),
                ...
                AppLifecycleCallbackHandler.getInstance().activity?.runOnUiThread {
                    ...
                    Toast.makeText(
                        activity,
                        String.format(activity!!.getString(R.string.log_level_default), sapWizardApplication.logUtil.getLevelString(finalLogLevel)),
                        Toast.LENGTH_SHORT).show()
                }
            }
        }
