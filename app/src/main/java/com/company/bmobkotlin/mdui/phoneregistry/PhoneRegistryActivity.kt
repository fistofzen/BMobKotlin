 ...
 * - [PhoneRegistryCreateFragment]: to create or edit a(n) PhoneRegistry,
 * - [PhoneRegistryDetailFragment]: to display the details of a(n) PhoneRegistry,
 * - [PhoneRegistryListFragment]: to list PhoneRegistry.
 *
 ...
 * what to do.
 */
class PhoneRegistryActivity : AppCompatActivity(), InterfacedFragment.InterfacedFragmentListener<PhoneRegistry> {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PhoneRegistryListFragment(), UIConstants.LIST_FRAGMENT_TAG)
                .commit()
        } else {
    ...
    override fun onFragmentStateChange(evt: Int, entity: PhoneRegistry?) {
        when (evt) {
    ...
    /**
     * Shows an AlertDialog when user wants to leave {@link PhoneRegistryCreateFragment}.
     */
    ...
    private fun onItemClicked(entity: PhoneRegistry?) {
        if (!isMasterDetailView) {
            supportFragmentManager.beginTransaction()
                .replace(R.id.masterFrame, PhoneRegistryDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
            ...
            if (detail == null) {
                if (entity != null)
                    supportFragmentManager.beginTransaction()
                        .replace(R.id.detailFrame, PhoneRegistryDetailFragment(), UIConstants.DETAIL_FRAGMENT_TAG)
                        .commit()
            } else {
                if( entity == null) {
    ...
    private fun onCreateNewItem() {
        ...
        val fragment = PhoneRegistryCreateFragment()
    ...
    @Suppress("UNUSED_PARAMETER")
    private fun onEditItem(entity: PhoneRegistry?) {
        val updateFragment = PhoneRegistryCreateFragment()
        val arguments = Bundle()
        ...
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity != null) {
                ...
                builder.setPositiveButton(R.string.before_navigation_dialog_positive_button) { _, _ ->
                    (fragmentActivity as PhoneRegistryActivity).onFragmentStateChange(UIConstants.EVENT_BACK_NAVIGATION_CONFIRMED, null)
                }
    ...
    internal class DelelteConfirmationDialogFragment : DialogFragment() {
        override fun onCreateDialog(savedInstanceState: Bundle?): Dialog {
            if (activity !=null) {
                val fragmentActivity = activity as AppCompatActivity
                val builder = AlertDialog.Builder(ContextThemeWrapper(fragmentActivity, R.style.AlertDialogStyle))
                val viewModel = ViewModelProviders.of(fragmentActivity).get(PhoneRegistryViewModel::class.java)
