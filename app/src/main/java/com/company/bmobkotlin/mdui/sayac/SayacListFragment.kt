...
/**
 * An activity representing a list of Sayac. This activity has different presentations for handset and tablet-size
 * devices. On handsets, the activity presents a list of items, which when touched, lead to a view representing
 * Sayac details. On tablets, the activity presents the list of Sayac and Sayac details side-by-side using two
 * vertical panes.
 */

class SayacListFragment : InterfacedFragment<Sayac>() {
    ...
    /**
     * List adapter to be used with RecyclerView containing all instances of sayacs
     */
    private var adapter: SayacListAdapter? = null
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        activityTitle = getString(EntitySetListActivity.EntitySetName.Sayac.titleId)
    ...
    override fun onViewStateRestored(savedInstanceState: Bundle?) {
        ...
        (currentActivity.findViewById<RecyclerView>(R.id.item_list))?.let {
            this.adapter = SayacListAdapter(currentActivity, it)
    ...
    private fun prepareViewModel() {
        viewModel = if( navigationPropertyName != null && parentEntityData != null ) {
            ViewModelProviders.of(currentActivity, EntityViewModelFactory(currentActivity.application, navigationPropertyName!!, parentEntityData!!))
                .get(SayacViewModel::class.java)
        } else {
            ViewModelProviders.of(currentActivity).get(SayacViewModel::class.java).also {
                it.initialRead()
            }
        }
        viewModel.observableItems.observe(this, Observer<List<Sayac>> { items ->
            items?.let { entityList ->
                adapter?.let { listAdapter ->
                    ...
                    item?.let {
                        viewModel.inFocusId = listAdapter.getItemIdForSayac(it)
                        if (currentActivity.resources.getBoolean(R.bool.two_pane)) {
                            viewModel.setSelectedEntity(it)
                            if(!isInActionMode && !(currentActivity as SayacActivity).isNavigationDisabled) {
     ...
     */
    private fun containsItem(items: List<Sayac>, item: Sayac) : Sayac? {
        return items.find { entry ->
            adapter?.getItemIdForSayac(entry) == adapter?.getItemIdForSayac(item)
        }
    }
    ...
    private fun setItemIdSelected(itemId: Int): Sayac? {
        viewModel.observableItems.value?.let { sayacs ->
            if (sayacs.isNotEmpty()) {
                adapter?.let {
                    viewModel.inFocusId = it.getItemIdForSayac(sayacs[itemId])
                    return sayacs[itemId]
                }
            }
        }
        return null
    ...
    private fun setDetailImage(viewHolder: SayacListAdapter.ViewHolder, sayacEntity: Sayac?) {
        if (isInActionMode) {
     ...
     */
    inner class OnActionModeStartClickListener(internal var holder: SayacListAdapter.ViewHolder) : View.OnClickListener, View.OnLongClickListener {
        ...
        private fun onAnyKindOfClick(): Boolean {
            val isNavigationDisabled = (activity as SayacActivity).isNavigationDisabled
            if (isNavigationDisabled) {
                Toast.makeText(activity, "Please save your changes first...", Toast.LENGTH_LONG).show()
            } else {
                if (!isInActionMode) {
                    actionMode = (currentActivity as AppCompatActivity).startSupportActionMode(SayacListActionMode())
                    adapter?.notifyDataSetChanged()
                }
     ...
     */
    inner class SayacListActionMode : ActionMode.Callback {
        override fun onCreateActionMode(actionMode: ActionMode, menu: Menu): Boolean {
            isInActionMode = true
            (currentActivity.findViewById<FloatingActionButton>(R.id.fab))?.let {
                it.hide()
            }
            //(currentActivity as SayacActivity).onSetActionModeFlag(isInActionMode)
        ...
        override fun onActionItemClicked(actionMode: ActionMode, menuItem: MenuItem): Boolean {
            return when (menuItem.itemId) {
                R.id.update_item -> {
                    val sayacEntity = viewModel.getSelected(0)
                    if (viewModel.numberOfSelected() == 1 && sayacEntity != null) {
                        isInActionMode = false
                        actionMode.finish()
                        viewModel.setSelectedEntity(sayacEntity)
                        if(currentActivity.resources.getBoolean(R.bool.two_pane)) {
                            //make sure 'view' is under 'crt/update',
                            //so after done or back, the right panel has things to view
                            listener?.onFragmentStateChange(UIConstants.EVENT_ITEM_CLICKED, sayacEntity)
                        }
    ...
    /**
    * List adapter to be used with RecyclerView. It contains the set of sayacs.
    */
    inner class SayacListAdapter(private val context: Context, private val recyclerView: RecyclerView) : RecyclerView.Adapter<SayacListAdapter.ViewHolder>() {

        /** Entire list of Sayac collection */
        private var sayacs: MutableList<Sayac> = ArrayList()

        /** Flag to indicate whether we have checked retained selected sayac */
        ...
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SayacListAdapter.ViewHolder {
        ...
        override fun getItemId(position: Int): Long {
            return getItemIdForSayac(sayacs[position])
        }

        override fun onBindViewHolder(holder: ViewHolder, position: Int) {
            checkForRetainedSelection()

            val sayacEntity = sayacs[holder.adapterPosition]
            (sayacEntity.getDataValue(Sayac.createdAt))?.let {
            holder.masterPropertyValue = it.toString()
            }
            populateObjectCell(holder, sayacEntity)

            val isActive = getItemIdForSayac(sayacEntity) == viewModel.inFocusId
            if (isActive) {
                setItemIdSelected(holder.adapterPosition)
            }
            val isSayacSelected = viewModel.selectedContains(sayacEntity)
            setViewBackground(holder.view, isSayacSelected, isActive)

            holder.view.setOnLongClickListener(OnActionModeStartClickListener(holder))
            setOnClickListener(holder, sayacEntity)

            setOnCheckedChangeListener(holder, sayacEntity)
            holder.isSelected = isSayacSelected
            setDetailImage(holder, sayacEntity)
        }

        /**
        * Check to see if there are an retained selected sayacEntity on start.
        * This situation occurs when a rotation with selected sayacs is triggered by user.
        */
        ...
        /**
        * Computes a stable ID for each Sayac object for use to locate the ViewHolder
        *
        * @param [sayacEntity] to get the items for
        * @return an ID based on the primary key of Sayac
        */
        internal fun getItemIdForSayac(sayacEntity: Sayac): Long {
            return sayacEntity.readLink.hashCode().toLong()
        }
        ...
        */
        private fun manageActionModeOnCheckedTransition() {
            if (actionMode == null) {
                actionMode = (activity as AppCompatActivity).startSupportActionMode(SayacListActionMode())
            }
        ...
        /**
        * This is called when one of the selected sayacs has been de-selected
        *
        ...
        */
        ...
        private fun populateObjectCell(viewHolder: ViewHolder, sayacEntity: Sayac) {
            ...
            viewHolder.objectCell.apply {
                ...
                when {
                sayacEntity.inErrorState -> setIcon(R.drawable.ic_error_state, 0, R.string.error_state)
                sayacEntity.isUpdated -> setIcon(R.drawable.ic_updated_state, 0, R.string.updated_state)
                sayacEntity.isLocal -> setIcon(R.drawable.ic_local_state, 0, R.string.local_state)
        ...
        private fun processClickAction(view: View, sayacEntity: Sayac) {
            resetPreviouslyClicked()
            setViewBackground(view, false, true)
            viewModel.inFocusId = getItemIdForSayac(sayacEntity)
        }
        ...
        */
        ...
        /**
        * If there are selected sayacs via long press, clear them as click and long press are mutually exclusive
        * In addition, since we are clearing all selected sayacs via long press, finish the action mode.
        */
        ...
        /**
        * Set up checkbox value and visibility based on sayacEntity selection status
        *
        * @param [checkBox] to set
        * @param [isSayacSelected] true if sayacEntity is selected via long press action
        */
        private fun setCheckBox(checkBox: CheckBox, isSayacSelected: Boolean) {
            checkBox.isChecked = isSayacSelected
        }
        ...
        */
        @WorkerThread
        fun setItems(currentSayac: List<Sayac>) {
            if (sayacs.isEmpty()) {
                sayacs = java.util.ArrayList(currentSayac)
                notifyItemRangeInserted(0, currentSayac.size)
            } else {
                val result = DiffUtil.calculateDiff(object : DiffUtil.Callback() {
                    ...
                    override fun getNewListSize(): Int {
                        return currentSayac.size
                    }

                    override fun areItemsTheSame(oldItemPosition: Int, newItemPosition: Int): Boolean {
                        return sayacs[oldItemPosition].readLink == currentSayac[newItemPosition].readLink
                    }

                    override fun areContentsTheSame(oldItemPosition: Int, newItemPosition: Int): Boolean {
                        val sayacEntity = sayacs[oldItemPosition]
                        return !sayacEntity.isUpdated && currentSayac[newItemPosition] == sayacEntity
                    }
                })
                sayacs.clear()
                sayacs.addAll(currentSayac)
        ...
        *
        * @param [holder] to set
        * @param [sayacEntity] associated with this ViewHolder
        */
        private fun setOnCheckedChangeListener(holder: ViewHolder, sayacEntity: Sayac) {
            holder.checkBox.setOnCheckedChangeListener { _, checked ->
                if (checked) {
                    //(currentActivity as SayacActivity).onUnderDeletion(sayacEntity, true)
                    viewModel.addSelected(sayacEntity)
                    manageActionModeOnCheckedTransition()
                    resetPreviouslyClicked()
                } else {
                    //(currentActivity as SayacActivity).onUnderDeletion(sayacEntity, false)
                    viewModel.removeSelected(sayacEntity)
                    manageActionModeOnUncheckedTransition()
                }
                setViewBackground(holder.view, viewModel.selectedContains(sayacEntity), false)
                setDetailImage(holder, sayacEntity)
            }
        }
        ...
        * @param [sayacEntity] associated with this ViewHolder
        */
        private fun setOnClickListener(holder: ViewHolder, sayacEntity: Sayac) {
            holder.view.setOnClickListener { view ->
                val isNavigationDisabled = (currentActivity as SayacActivity).isNavigationDisabled
                if( !isNavigationDisabled ) {
                    resetSelected()
                    resetPreviouslyClicked()
                    processClickAction(view, sayacEntity)
                    viewModel.setSelectedEntity(sayacEntity)
                    listener?.onFragmentStateChange(UIConstants.EVENT_ITEM_CLICKED, sayacEntity)
                } else {
        ...
        /**
        * Set background of view to indicate sayacEntity selection status
        * Selected and Active are mutually exclusive. Only one can be true
        *
        * @param [view]
        * @param [isSayacSelected] - true if sayacEntity is selected via long press action
        * @param [isActive]           - true if sayacEntity is selected via click action
        */
        private fun setViewBackground(view: View, isSayacSelected: Boolean, isActive: Boolean) {
        ...
        */
        inner class ViewHolder(val view: View) : RecyclerView.ViewHolder(view) {
            ...
            /** Fiori ObjectCell to display sayacEntity in list */
