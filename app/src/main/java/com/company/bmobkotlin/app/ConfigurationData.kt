        ...
        try {
            ...
            if (configData.length() == 0) {
                ...
                when (configData.has(KEY_SERVICE_URL)) {
                    // Use the service URL format
                    true -> {
                        // Make sure the last Char of serviceUrl is '/'
                        serviceUrl = configData.getString(KEY_SERVICE_URL)?.let {
                            if (it.last() != '/') {
                                it + "/"
                            }
                            else {
                    ...
                    // Use the discovery service format
                    false -> {
                        val host = configData.getString(KEY_DISC_SVC_DFLT_HOST)?.let {
                            // Make sure HOST does not end with "/"
                            if (it.last() == '/') {
                                it.dropLast(1)
                            }
                            else {
